# BTCert
BTCert is a blockchain based project for protecting authentic qualifications (academic certificate). The project is inspired by [Blockcerts](http://www.blockcerts.org/) and based on its open standard. BTCert employed the same principle with Blockcerts. BTCert releases the certificates by sending a Bitcoin transaction from awarding institution to the recipient. A good metaphor is illustrated in Figure 1, a set of hash value of certificates would be attached to the Bitcoin transaction when Alice paid 5 BTC to Tom. By the design of BTCert, Alice replaces the note "thank you" by the Merkle root which generated by a group of certificates. Then, BTCert allows an independent verifier to verify the authenticity of such certificates by accessing hash value on the blockchain and comparing it with the local.

![](src/main/resources/static/images/metaphor.png)
*Figure 1. Working mechanism of Blockcerts*

Also, BTCert employed some new cryptographic technologies to make it more secure.These cryptographical protocols include using multiple signature approaches to improve certificate authentication, exerting a safe revocation mechanism based on BTC address to revoke a certificate more reliably, and establishing a secure federated identity aimed at verifying certificate awarding institution's identity. Meanwhile, the core cryptographic parts of BTCert are implemented by JavaScript, which made it possible to handle all the sensitive data client side (browser). In another word, BTCert never transfers and stores any sensitive data such as the private keys on a server side, that made the data more secure.
