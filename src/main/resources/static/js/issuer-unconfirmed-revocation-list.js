...
if(items.length == 1){
    ...
    if(items.length == 1){
    	...
    	id  = items[0].id;
    	 $.getJSON("/issuer/getRevokeTansacInfo/" + id,function(result){
    	        $.each(result,
    	            function(name, item) {
    	        	if(name =='input'){
    	        		...
    	        		if(item){
    	        			...
    	        			for(var i = 0 ; i < sigInfo.length - 1; i++){
    	        				...
    	        				sign_state_html = sign_state_html + "<tr><td>"+shtml[0]+"</td><td>"+shtml[1]+"</td><td>"+shtml[2]+"</td><td>"+shtml[3]+"</td></tr>"
    	        			}
...
function confirmTrsanction(confirmResult){
	
	var raw_tran = "";
	if(confirmResult == 'agree'){
		 ...
		 * var txous = Object.create(btcert._tranction_output);
		 * 
		 * btcert._tranction_output = []; for (var i = 0 ; i <
		 * txous.length; i++){ var txou = {scriptHex:
	...
	var sign_state ="_"+confirmResult + "_"+ $("#opinion").val(); // 签发已经
    ...
    var queryConditionStr = JSON.stringify(queryConditionJson);
     ...
     $.ajax({
        url: "/issuer/signRevokeTansacInfo",
        dataType: 'json',
        type: 'post',
        contentType: 'application/json',
        data: queryConditionStr,
        ...
        success: function( result, textStatus, jQxhr ){
        	  if(result.success == "1"){
                    swal({
                        title: "Good Job!",
                        text: "Confirmed Revocation successfully!.",
                        type: "success",
                    ...
                    function(){
                        window.location.href = "/issuer/confirmed_revocation_list";
                    });
                }else{
...
var TableInit = function () {
    ...
    oTableInit.queryParams = function (params) {
        var queryConditionJson = {
        		...
        		'queryObject':'{"sign_state":"notExists_'+student_name+'","broadcast_state":"0","id":"matchAll_'+params.searchText+'"}',
        		'rowCount':1,
